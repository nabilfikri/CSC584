...
<jsp:useBean id="timeBeanId" class="chapter9.TimeBean"
	scope="application">
</jsp:useBean>
<html>
<head>
...
<body>
	...
	<form method="post" action="DisplayTime.jsp">
		Locale <select size="1" name="localeIndex">
			...
			<%}%>
		</select><br /> Time Zone <select size="1" name="timeZoneIndex">
			<% for (int i = 0; i < timeBeanId.getAllTimeZone().length; i++) {%>
			<option value="<%= i %>">
				<%= timeBeanId.getAllTimeZone()[i] %>
			</option>
			<%}%>
		</select><br /> <input type="submit" name="Submit" value="Get Time" /> <input
